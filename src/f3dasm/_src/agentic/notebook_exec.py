...
def notebook_deliverable_spec(role: str = "strategizer") -> str:
    ...
    return (
        ...
        "  Read the first NON-provenance output column from the loaded data:\n"
        "  `output_col = next(c for c in data.domain.output_names if not c.startswith('_'))`.\n"
        "  (The ledger stores provenance columns `_delegation_id`, `_source`, `_ts`,\n"
        "   `_wall_ms`; the domain SORTS output names, so the `_`-prefixed ones sort\n"
        "   FIRST — `output_names[0]` is metadata, not the objective. Skip the `_` ones.)\n"
