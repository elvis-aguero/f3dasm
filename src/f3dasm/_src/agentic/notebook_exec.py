...
def notebook_deliverable_spec(role: str = "strategizer") -> str:
    ...
    return (
        ...
        "  immediately re-call AddPipelineCell or PatchNotebookCell to update both the\n"
        "  hypotheses markdown cell and the analysis cell BEFORE calling CheckDeliverable.\n"
        ...
        "- CELL SCOPE: every cell that needs the canonical store path must define it\n"
        ...
        "  NEVER use `__file__` as a fallback, and NEVER supply any string default to\n"
        "  `.get()` — the gate always injects this variable, so any fallback is dead code\n"
        "  that bakes in a developer path and breaks portability. Use square-bracket access:\n"
        "  `canonical_store = os.environ['F3DASM_CANONICAL_STORE']` (raises if missing).\n"
