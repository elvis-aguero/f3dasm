...
class AgenticRun:
    ...
    def _make_adapter(self, name: str, agent: Agent):
        ...
        # The deliverable is pipeline.ipynb. Inject its contract into the agents
        # that author/judge it (strategizer assembles it, the implementer
        # contributes phase code, the critic judges it).
        from .notebook_exec import notebook_deliverable_spec
        if getattr(agent, "role", None) in (
