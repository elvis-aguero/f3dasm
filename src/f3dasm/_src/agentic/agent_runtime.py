...
class AgenticRun:
    ...
    def execute(self) -> str:
        ...
        # Collect replicate.py from debug/delegations/ if agent wrote one
        replicate_py_content = ""
        _delegations_glob_dir = Path(run_dir) / "debug" / "delegations"
        if _delegations_glob_dir.exists():
            replicate_candidates = sorted(
                _delegations_glob_dir.glob("**/replicate.py"),
        ...
        if replicate_py_content:
            # Inject provenance header (plumbing responsibility, not agent's)
            ...
            (self.study_dir / "replicate.py").write_text(
                header + replicate_py_content, encoding="utf-8"
            )
