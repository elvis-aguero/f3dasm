...
# ---------------------------------------------------------------------------
# pipeline.py provenance header
# ---------------------------------------------------------------------------


@pytest.fixture
def pipeline_run_with_pipeline(tmp_path):
    """Like pipeline_run but worker writes a pipeline.py to its sandbox."""
    study = _make_study(tmp_path)
    strat = ScriptedStrategistAdapter()
    ...
    class ReplicateWritingWorker(MockWorkerAdapter):
        def invoke(self, messages):
            ...
            if write is not None:
                ...
                write("pipeline.py", "import f3dasm\nprint('hello')\n")
            return self._report
...
def test_pipeline_py_has_provenance_header(pipeline_run_with_pipeline):
    """pipeline.py at study root starts with the auto-generated provenance header."""
    _, study = pipeline_run_with_pipeline
    pipeline = study / "pipeline.py"
    assert pipeline.exists(), "pipeline.py not written to study root"
    content = pipeline.read_text(encoding="utf-8")
    assert content.startswith("# Auto-generated by agentic-f3dasm"), (
        f"pipeline.py does not start with provenance header; got: {content[:100]!r}"
    )
